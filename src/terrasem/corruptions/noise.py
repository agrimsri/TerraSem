"""Noise corruption for sensor degradation robustness evaluation.
...
def apply(data: np.ndarray, severity: int, seed: int = 42) -> np.ndarray:
    """Apply noise corruption at given severity (0=no-op, 1-5 increasing).

    TODO (Phase 7): implement.
    """
    if severity == 0:
        return data
    raise NotImplementedError("noise corruption not yet implemented (Phase 7).")
